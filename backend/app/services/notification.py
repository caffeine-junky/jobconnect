...
class NotificationService:
    ...
    def notification_in_db_to_response(self, notification: NotificationInDB) -> NotificationResponse:
    ...
    async def create_notification(self, data: NotificationCreate) -> NotificationResponse:
        """"""
        notification: Optional[NotificationInDB] = await self.repo.create(data.model_dump())
    ...
    async def read_notification(self, notification_id: UUID) -> NotificationResponse:
        """"""
        notification: Optional[NotificationInDB] = await self.repo.readone(notification_id)
        if notification is None:
    ...
    async def readall_notifications(
        ...
        """"""
        notifications: List[NotificationInDB] = await self.repo.readall(read, skip, limit)
        return [self.notification_in_db_to_response(notification) for notification in notifications]
    ...
    async def readall_notifications_by_client_id(
        ...
        """"""
        notifications: List[NotificationInDB] = await self.repo.readall_by_client_id(client_id, read, skip, limit)
        return [self.notification_in_db_to_response(notification) for notification in notifications]
    ...
    async def readall_notifications_by_techniian_id(
        ...
        """"""
        notifications: List[NotificationInDB] = await self.repo.readall_by_technician_id(technician_id, read, skip, limit)
        return [self.notification_in_db_to_response(notification) for notification in notifications]
